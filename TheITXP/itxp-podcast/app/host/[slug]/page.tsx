...
export async function generateStaticParams() {
  return getContentSlugs("host").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const host = await getContentItem("host", slug);
  if (!host) return {};
  const name = host.frontMatter.Title ?? host.frontMatter.title ?? slug;
...
export default async function HostPage({ params }: Props) {
  ...
  const { frontMatter, contentHtml } = host;
