...
- **Premium logo redesign**: New star/compass + horizon icon design (selected from Higgsfield generations, ID 808601b3-3e79-4122-b232-5a353d426c69). Includes dark/light + lockup/horizontal variants as PNGs for better fidelity. Integrated into hero, nav, footer, favicon, 404 mark. Old placeholder SVGs retired in main usage.
  - Recent polish/fixes (after live review): Simplified hero brand to *single* instance of the selected design (no full/compact split that could duplicate). Hid nav logo on homepage (via `bodyClass: 'home'` + `.home .nav-logo { display: none }`) to avoid too many logos clustered at the top. Stripped any baked-in solid/dark backgrounds from all PNGs via edits for true transparency (prevents contrast issues on navy hero/footer). Adjusted hero brand container size + mobile/desktop rules for better fit and no overlap. Bumped CSS @import versions + assetVersion for cache bust. Now only one clean logo visual in the hero spot on home.
